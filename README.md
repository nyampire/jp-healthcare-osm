...
| タグ | 対象業態 | 生成数 |
|---|---|---:|
| `amenity` | 助産所以外 | 194,615 |
| `healthcare` | 全業態 | 196,401 |
| `name` / `official_name` | 全業態 | 196,401 |
| `short_name` | 略称のある業態 | 95,195 |
| `name:ja-Hira` | 全業態 | 150,550 |
| `name:ja-Latn` | 全業態 | 76,097 |
| `name:en` | 全業態 | 81,546 |
| `opening_hours` | 全業態 | 190,562 |
| `healthcare:speciality` | 病院、診療所、歯科診療所 | 137,260 |
| `website` | 全業態 | 121,931 |
| `addr:full` | 全業態 | 196,401 |
| `addr:country` | 全業態 | 196,401 |
| `addr:province` | 全業態 | 196,401 |
| `addr:county` | 郡がある住所 | 11,632 |
| `addr:city` | 全業態 | 196,389 |
| `addr:suburb` | 政令指定都市の区など | 47,499 |
| `addr:quarter` | 町字の下に区分がある住所 | 8,599 |
| `addr:neighbourhood` | 全業態 | 192,777 |
| `addr:block_number` | 住居表示を実施した区域 | 113,629 |
| `addr:housenumber` | 住居表示を実施した区域 | 112,450 |
| `note` | 住所に未解釈の部分が残った施設 | 65,973 |
| `fixme` | 番地の要素が3つ以上ある施設 | 3,362 |
| `beds` | 病院、診療所 | 11,677 |
| `source` | 全業態 | 196,401 |
...
都道府県ごとに分割した line-by-line GeoJSON を、233チャレンジ196,401タスク分出力します。
...
206,043施設のうち196,401件を出力しています。

| 業態 | 施設数 | 出力 | 座標が元データ | 座標を付与 | 座標なしで除外 |
|---|---:|---:|---:|---:|---:|
| 病院 | 7,715 | 7,526 | 7,436 | 90 | 189 |
| 診療所 | 80,155 | 77,117 | 75,044 | 2,073 | 3,038 |
| 歯科診療所 | 54,637 | 52,617 | 51,288 | 1,329 | 2,020 |
| 助産所 | 2,138 | 1,786 | 1,666 | 120 | 352 |
| 薬局 | 61,398 | 57,355 | 53,977 | 3,378 | 4,043 |
| 合計 | 206,043 | 196,401 | 189,411 | 6,990 | 9,642 |

「座標を付与」6,986件のうち286件は、元データに座標がありながら
ジオコーダ座標から1km以上離れていたため、ジオコーダ座標を採ったものです。
判定の条件は [docs/processing.md](docs/processing.md) の
「元データの座標がジオコーダ座標から離れている場合」にまとめています。

除外した9,642件の内訳は、地番方式の住所が5,187件、位置レベルが足りないものが4,455件です。
...
これに、元データが穴埋めに入れた座標を捨てて住所からも補えなかった99件が加わり、座標を持たない施設は9,642件です。
5,143件は地番方式の住所です。
