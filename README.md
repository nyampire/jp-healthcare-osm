...
| `addr:neighbourhood` | 誤った町字が出た行を除く全業態 | 192,672 |
| `addr:block_number` | 住居表示の明細と照合できた行 | 88,113 |
| `addr:housenumber` | 住居表示の明細と照合できた行 | 88,113 |
| `beds` | 病院、診療所 | 11,677 |
| `source` | 全業態 | 196,680 |

入力とは別の大字が町字として出た384行には `addr:neighbourhood` を付けていません。

NJA は `○○町` の `町` を落とした別名を町字パターンとして登録しています。
入力が指す町字を照合できたときはそちらが勝ちますが、照合できなかったときだけ
別名が最後に拾い、「町字を特定できず」ではなく「誤った町字」になります。
`神奈川県横浜市鶴見区鶴見中央` が `鶴見町` になり、`中央` が残余に落ちる形です。
元データ206,043行のうち401行で起きており、うち384行が出力に残ります。

誤った町字は OSM 上で別の場所を指すため、出さないほうがましと判断しました。
どの行がそれかは町字マスタに照会して決めています（`scripts/town_overmatch.js`）。
正しい町字の候補は備考と MapRoulette のタスクに回し、作業者に見せます。
元の住所文字列は `addr:full` に全件残るので、情報は失われません。
...
それは作業者が見られる場所に残っていれば足ります。元の住所は `addr:full` に載るので
あとから参照もできます。
