...
def main():
    ...
    eq("略称が無くても施設種別語だけになるなら戻す",
       strip("楠井 歯科", ""), ("楠井歯科", [], []))
    eq("推定で落とした語が複数でもまとめて戻す",
       strip("すえおか こども クリニック"),
       ("すえおかこどもクリニック", [], []))
    eq("戻すときに法人格は戻さない",
       strip("医療法人社団 すえおか こども クリニック"),
       ("すえおかこどもクリニック", ["医療法人社団"], []))
    eq("施設種別語が連なっていても戻す",
       strip("田中 歯科クリニック"), ("田中歯科クリニック", [], []))
    eq("運営主体の語も施設種別語だけになるなら戻す",
       strip("市立 診療所"), ("市立診療所", [], []))
    eq("戻すとき欧字どうしの間の空白は残す",
       strip("OKP with Life クリニック"),
       ("OKP with Lifeクリニック", [], []))
    eq("施設種別語だけにならない行は従来どおり落とす",
       strip("楠井 小児科クリニック", ""),
       ("小児科クリニック", ["楠井"], ["楠井"]))
    ...
    # 法人名の接尾辞で終わるトークンも、落とすと施設種別語だけが残ることがある。
    # 推定ではないので戻す処理には入らず、落とす前に止める。
    eq("法人名を落とすと施設種別語だけになるなら落とさない",
       strip("晋栄福祉会 診療所"), ("晋栄福祉会診療所", [], []))
    eq("法人格を落とした後も同じ判定をする",
       strip("医療法人たんぽぽ会 歯科"),
       ("たんぽぽ会歯科", ["医療法人"], []))
    eq("略称が残りだけを名乗るなら法人名を落とす",
       strip("仁寿会 荘病院", "荘病院"), ("荘病院", ["仁寿会"], []))
